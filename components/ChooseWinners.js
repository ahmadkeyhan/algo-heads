...
function ChooseWinner() {
  ...
  function chooseWinner() {
    for (var i = 1; i < 6; i++) {
      var winnerIndex = Math.floor(Math.random()*shuffleArray.length)
      var winner = shuffleArray.splice(winnerIndex,1)
      fetch('api/winners' , {
        method: 'POST',
        body: JSON.stringify({asset: `AH10${i}`, winner: winner})
      }).then((res) => res.json())
      .then(() => {
        console.log(`AH10${i} winner is ${winner}`)
      })
    }
  }
